...
def replace_by_default_value(x, default_values=None):
    """
    Replace the remaining -999 by the a default value
    ...
    if default_values is None:
        # If no default values specified (i.e. we clean the training set), compute the mean for each column and
        # replace -999 by this value
...
def percentile_comparison(row, above_lim, below_lim):
    """
    Clip row value to the above or below the given limits
    Parameters
    ----------
        row : 
...
def clip_IQR(x, k=1.5, percentiles=[25, 75], above_lim=None, below_lim=None):
    ...
    if above_lim is None or below_lim is None:
        ...
        # Compute interquartile range and the limits
        iqr = (perc[1, :] - perc[0, :])
        above_lim = perc[1, :] + k * iqr
        below_lim = perc[0, :] - k * iqr

    # Clip the value in every row 
    x = np.apply_along_axis(lambda row: percentile_comparison(row, above_lim, below_lim), 1, x)
...
def standardise(x):
    """
        Standardise the original data set according to the formula : (x-mean(x))/std(x).
    Parameters
    ----------
        x :
            Data points to standardise
    Returns
    -------
        x : 
            Data points standardised
        mean_x :
            Mean of x before standardisation
        std_x :
